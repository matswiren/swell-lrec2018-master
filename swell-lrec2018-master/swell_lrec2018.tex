...
\dan{State the contributions of the paper?
Our contributions are:
\begin{itemize}
\item An editor for norm deviations that works by transforming a text and as a side product produces a
parallell corpus (Section~\ref{sec:norm_tool})
\item By viewing normalisation as a parallell corpus, we show that we can label using a
norm deviation taxonomy on the aligment edges (Section~\ref{sec:ann_tool})
\end{itemize}}
